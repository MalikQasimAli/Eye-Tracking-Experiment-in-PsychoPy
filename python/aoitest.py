...
#   attrstr = aoi.getAttribute_astring('Value')
#   print(stim + ',' + attrstr,file=df)

    attrnam = aoi.getAttribute_astring('Name')
    attrval = aoi.getAttribute_astring('Value')
    attrord = int(attrnam[1:]) if attrnam[0] == 'a' else 0
    outstr = "%s,%d,%s,%s" % (stim,attrord,attrnam,attrval)
    print(outstr,file=df)
...
# print("in" if aoi.inside(0.0,0.0) else "out")
  ...
  # process AOI file
  aoidir = "trial_aoi/"
# aoidir = "../../../aois/1920x1080/lines/PL/"
# aoifile = aoidir + "museum_signage.sla"
...
#         print(path, top, dname, base, "[",basename,"]","[",file,"]")
  ...
  df = open("aois.csv",'w')
  print("stim,aoi_order,aoi_name,aoi_label",file=df)

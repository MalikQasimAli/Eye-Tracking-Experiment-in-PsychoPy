...
#   print("pdwtlevel: len(d), dwtlvl: ",len(d),dwtlvl)
...
#     ly = "%c" % (chr(int(iy+97)))
 ...
 #      print "%f, idx = %d" % (pix[j,i], idx)
  ...
  def dumpFixatedAOIs(self,fileName,w,h,aoilist):
    ...
    str = "timestamp,x,y,duration,prev_sacc_amplitude,aoi_span,aoi_order,aoi\n"
    outfile.write(str)
    i=0
    span=0
    prev_fixation=0
    ...
    for fx in self.fixations:
      ...
      if inAOI:
        ...
        span = 0 if prev_aoi == 0 else aoi.getLocation() - prev_aoi
        prev_aoi = aoi.getLocation()
        prev_sacc_amp = 0.0
        if prev_fixation > 0:
        ...
        str = "%f,%f,%f,%f,%f,%s,%s,%s\n" % \
               (fx.gettimestamp(),x,y,\
                fx.getDuration(),prev_sacc_amp,\
                span,aoi.getLocation(),\
                aoi.getAttribute_astring('Value'))
        outfile.write(str)
        prev_fixation=i
      i += 1
    outfile.close()

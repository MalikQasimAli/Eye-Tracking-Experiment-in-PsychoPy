...
def usage():
  print("Usage: python hdf52csv.py " \
        " --indir=? -outdir=?\n" \
        " --width=? -height=?\n" \
        " --file=?\n" \
...
def hdf52csv(infile,outdir,width,height,dist):
...
#    ses_id = row['session_id']
  ...
  for msg in mestable.iterrows():
    ...
    if 'trial' in msg['category'].decode('utf-8') and \
       'start' in msg['text'].decode('utf-8'):
      ...
      # get the actual category and text strings, e.g.,
      # category == trial_question text == start:tc04
      category = msg['category'].decode('utf-8')
      text = msg['text'].decode('utf-8')
      stim = text.split(':')[1]
      if 'question' in category:
        stim = stim[:2] + 'q' + stim[2:]
      elif 'answer' in category:
        stim = stim[:2] + 'a' + stim[2:]

    # get record info: second of pair of lines should be 'end'
    if 'trial' in msg['category'].decode('utf-8') and \
       'end' in msg['text'].decode('utf-8'):
      et = msg['time']
      ...
      outfilename = "%s-%s-%s-%s.csv" % (subj,exp_id,ses_id,stim)
...
# pbar = tqdm(total=len(files))
  ...
  for file in files:
    hdf52csv(file,outdir,width,height,dist)
    # update progress bar
#   pbar.update(1)
